/*
  This builds for cp/m on intel 8080 by first building for cp/m using z88dk and -clib=8080, then decompiling the binaries,
  converting them from 8080 assembly to 8086 assembly with 8088ify then recompiling with NASM
*/
...
function start () {
  ...
  execSync(
    'mkdir -p build;' +
    'mkdir -p build/8080;' +
    'mkdir -p build/8080/bin;' +
    'mkdir -p build/8080/s;' +
    'mkdir -p build/8080/g;' +
    'mkdir -p build/8080/h;' +
    'mkdir -p build/8080/ro;' +
    'mkdir -p build/8080/obj;' +
    'mkdir -p build/8080/obj2;' +
    'mkdir -p build/8080/fcb;' +
    'mkdir -p output8080;' +
    'rm -f build/8080/rodata2.asm;' +
    'rm -f build/8080/data.asm;' +
    'touch build/8080/data.asm'
  );
...
function compileLexer () {
  ...
  execSync(
    'sed "' +
      ...
      '" lexer.c > build/8080/lexer2.h'
  );

  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ lexer2.c -E -o build/8080/lexer3.c && ' +
      '../../makeheaders -h build/8080/lexer3.c | grep -v __LIB__ | grep -v extern | grep -v \\#define | sort | uniq > build/8080/lexer3.h'
  );

  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ build/8080/lexer3.c -S -o build/8080/lexer.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0lexer/g" build/8080/lexer.asm'
  );
...
function compileParser () {
  ...
  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ sql2.c -E -o build/8080/sql3.c && ' +
      '../../makeheaders -h build/8080/sql3.c | grep -v __LIB__ | grep -v extern | grep -v \\#define | sort | uniq > build/8080/sql3.h'
  );

  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ sql2.c -S -o build/8080/sql.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0sql/g" build/8080/sql.asm'
  );
...
function compileLibC2 () {
  ...
  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ libc2.c -S -o build/8080/libc2.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0libc2/g" build/8080/libc2.asm'
  );
...
function compileQueryCSV () {
  ...
  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ querycsv.c -S -o build/8080/querycsv.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0querycsv/g" build/8080/querycsv.asm'
  );
...
function compileHash2 () {
  ...
  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ hash2dat.c -S -o build/8080/hash2.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0hash2/g" build/8080/hash2.asm'
  );
...
function compileHash3 () {
  ...
  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ hash3.c -S -o build/8080/hash3.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0ghash3/g" build/8080/hash3.asm'
  );
  ...
  execSync(
    'sed "' +
      ...
      's/return \\&wordlist\\[key\\];/' +
        ...
        '}/gi;' +
    '" hash4a.c > build/8080/hash4a.c'
  );

  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ build/8080/hash4a.c -S -o build/8080/hash4a.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0hash4a/g" build/8080/hash4a.asm'
  );
  ...
  execSync(
    'sed "' +
      ...
      's/return \\&wordlist\\[key\\];/' +
        ...
        '}/gi;' +
    '" hash4b.c > build/8080/hash4b.c'
  );

  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ build/8080/hash4b.c -S -o build/8080/hash4b.asm;' +
      'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0hash4b/g" build/8080/hash4b.asm'
  );
  ...
  execSync(
    'sed "' +
      ...
      's/return \\&wordlist\\[key\\];/' +
        ...
        '}/gi;' +
    '" hash4c.c > build/8080/hash4c.c'
  );

  execSync(
    'zcc +cpm -clib=8080 -O0 --c-code-in-asm -Cc-params-offset=6 -U__STDC_VERSION__ build/8080/hash4c.c -S -o build/8080/hash4c.asm;' +
    'sed -i -E "s/\\bi_[0-9]+(_i_[0-9]+)?\\b/\\0hash4c/g" build/8080/hash4c.asm'
  );
...
function addROData () {
  ...
  execSync(
    'mv build/8080/ro/i_1hash4*.asm build/8080/ && ' +
    'cat build/8080/ro/i_1*.asm >> build/8080/rodata.asm && ' +
    'echo "  SECTION code_compiler" >> build/8080/rodata.asm && ' +
    'rm build/8080/ro/i_1*.asm && ' +
    'mv build/8080/i_1hash4*.asm build/8080/ro/'
  );

  execSync('z88dk-z80asm -b build/8080/rodata.asm');

  rodataSize = fs.statSync('build/8080/rodata.bin').size;
  ...
  execSync(`z88dk-z80asm -b -m -r=${32768 - rodataSize} build/8080/rodata.asm`);

  /* add the address of each rodata item as an assembly include file for anything that may need to reference it later */
  fs
    .readFileSync('build/8080/rodata.map', 'utf8')
    .replace(/(i_1[a-zA-Z0-9]+)[^$]+\$([0-9a-fA-F]+)/g, (one, two, three, ...arr) => {
      const text = 'IFNDEF ' + two + '\n' +
            two + ' = $' + three + '\n' +
          'ENDIF\n';
      fs.writeFileSync(
          `build/8080/ro/${two}.asm`,
          text,
          'utf8'
      );

      fs.appendFileSync('build/8080/rodata2.asm', text);
    });

  /* de-duplicate the global variables */
  execSync(
    'sort build/8080/globals.asm | uniq > build/8080/globals2.asm && ' +
    'rm build/8080/globals.asm && ' +
    'mv build/8080/globals2.asm build/8080/globals.asm'
  );

  var list = [];
  var walker = walk.walk('./build/8080/s', {});

  walker.on('file', (root, fileStats, next) => {
    list.push(
      [fileStats.name, fs.readFileSync('build/8080/s/' + fileStats.name, { encoding: 'utf8' })]
    );
...
function getFunctionSizes () {
  ...
  name = 'compareCommon';
  execSync(
    'cat build/8080/s/compareCodepoints.asm >> build/8080/s/getBytes.asm;' +
    "sed -i 's/_compareCodepoints/_" + name + "/g;s/querycsv/querycsv1/g;' build/8080/s/getBytes.asm"
  );
  ...
  execSync('rm build/8080/s/compareCodepoints.asm');

  name = 'sortCodepoints';
  execSync(
    'cat build/8080/s/sortCodepoints.asm >> build/8080/s/getBytes.asm;' +
    "sed -i 's/querycsv/querycsv1/g;' build/8080/s/getBytes.asm"
  );
  ...
  name = 'sortCodesParse';
  execSync(
    'cat build/8080/s/sortCodepoints.asm >> build/8080/s/parse_mbcs.asm;' +
    "sed -i 's/_sortCodepoints/_" + name + "/g;' build/8080/s/parse_mbcs.asm"
  );
  ...
  execSync('rm build/8080/s/sortCodepoints.asm');

  name = 'sortBytesParse';
  execSync(
    'cat build/8080/s/sortBytes.asm >> build/8080/s/parse_mbcs.asm;' +
    "sed -i 's/_sortBytes/_" + name + "/g;s/querycsv/querycsv2/g;' build/8080/s/parse_mbcs.asm"
  );
  ...
  name = 'sortBytes';
  execSync(
    'cat build/8080/s/sortBytes.asm >> build/8080/s/getCodepointsMbcs.asm;' +
    "sed -i 's/querycsv/querycsv3/g;' build/8080/s/getCodepointsMbcs.asm"
  );
  ...
  execSync('rm build/8080/s/sortBytes.asm');

  /* compile the data immediately above the function jump table */
  execSync('z88dk-z80asm -b -r=49152 build/8080/data.asm');

  spawnSync(
    'sh',
    [
      '-c',
      "z88dk-z80asm -m -b -r=`ls -nl build/8080/data.bin | awk '{print " + currentAddr + "}'` build/8080/data.asm"
    ],
    {
      stdio: 'inherit'
    }
  ...
  fs
    .readFileSync('build/8080/data.map', 'utf8')
  ...
  const walker = walk.walk('./build/8080/s', {});
      ...
      list
        ...
        .reduce((obj, elem) => {
          elem.children = elem.children
            ...
            .filter(name => fs.existsSync(path.join(__dirname, 'build/8080', 'obj', name + '.bin')));

          try {
            elem.size = fs.statSync(path.join(__dirname, 'build/8080', 'obj', elem.name + '.bin')).size;
          } catch (e) {}
...
function compilePages (pages) {
  ...
  pages.forEach((elem, index) => {
    addDefines('qrycsv' + (index + 1), elem.map(elem2 => elem2.name), 'h', true);

    fs
      .readFileSync('build/8080/obj2/qrycsv' + (index + 1) + '.map', 'utf8')
      .replace(/(^|\n)([_a-zA-Z0-9]+)[^$]+\$([0-9a-fA-F]+)/g, (one, blah, two, three, ...arr) => {
    ...
    if (fs.statSync('build/8080/obj2/qrycsv' + (index + 1) + '_code_compiler.bin').size > (16384 - rodataSize)) {
      console.log('page ' + (index + 1) + ' is too big');
      process.exit(-1);
    }

    execSync('dd if=/dev/zero bs=1 count=16384 of=build/8080/obj2/qrycsv' + (('00' + (index + 1)).substr(-2)) + '.ovl');

    execSync('dd if=build/8080/obj2/qrycsv' + (index + 1) + '_code_compiler.bin of=build/8080/obj2/qrycsv' +
    (('00' + (index + 1)).substr(-2)) + '.ovl conv=notrunc');

    execSync('dd if=build/8080/rodata.bin of=build/8080/obj2/qrycsv' +
    (('00' + (index + 1)).substr(-2)) + '.ovl bs=1 seek=' + (16384 - rodataSize) + ' conv=notrunc');
  });

  execSync('rm build/8080/obj2/*.bin');
}
...
function compileLibC (pages) {
  ...
  for (i = 0, j = pages.length + 1; i < j; i++) {
    foo += 'defb 0b11111111 ; ' + (i + 1) + '\n';
  }

  // build the asm includes
  ['fcb'].forEach((name, index) => {
    execSync('cp libc.c pager8080.asm serialLnBC.asm Makefile build/8080/' + name + '/;cp querycsv.h en_gb.h gen.h build/');

    fs.writeFileSync('build/8080/' + name + '/defines.inc',
    ...
    fs.writeFileSync('build/8080/' + name + '/lookupTable.inc', functionsList.map(item =>
       (item[1] === 1 ? '  GLOBAL ' : '  ;') + item[0] + '\n' +
      '  defw 0x' + ('0000' + item[3].toString(16)).substr(-4).toUpperCase()
    ).join('\n'));

    fs.writeFileSync('build/8080/' + name + '/functions.inc', functionsList.map(item =>
      (item[0] === 'main' ? '  PUBLIC _main\n_main:\n' : '') +
      '  call ' + item[4] + '  ;' + item[0] + '\n  defb ' + (item[1] === 1 ? index + 1 : item[1])
    ).join('\n'));

    fs.writeFileSync('build/8080/' + name + '/pages.inc', foo);
  });
...
function compileLibC3(fixDefines) {
  ['fcb'].forEach((name, index) => {
    spawnSync(
      ...
      {
        stdio: 'inherit',
        cwd: path.resolve(__dirname, 'build/8080', name)
      });
    ...
    if(fixDefines) {
      ...
      */
      fs
        .readFileSync('build/8080/' + name + '/qrycsv1a.map', 'utf8')
        .replace(/(^|\n)([_a-zA-Z0-9]+)[^$]+\$([0-9a-fA-F]+)/g, (one, blah, two, three, ...arr) => {
          const four = two.replace(/^_/, '');

          if(four === 'funcstart') {
            execSync(`sed -i '1s/^/defs ${startOffset - parseInt(three, 16)}, 0\\n/' build/8080/${name}/defines.inc`);
          }
        });
    }

    execSync('rm -f build/8080/' + name + '/qrycsv1a.ovl');
    ...
    if(!fixDefines) {
      execSync('dd if=/dev/zero bs=1 count=16128 of=build/8080/obj2/qrycsv1a.ovl');

      execSync('dd if=build/8080/' + name + '/qrycsv1a.ovl of=build/8080/obj2/qrycsv1a.ovl conv=notrunc;cp build/8080/fcb/*.map build/8080/obj2/');
    }
...
function splitUpFunctions (filename, callback, append) {
  ...
  const lineReader = readline.createInterface({
    input: fs.createReadStream('build/8080/' + filename + '.asm')
  });

  const data = fs.createWriteStream('build/8080/data.asm', {
    flags: 'a'
  });

  const globals = fs.createWriteStream('build/8080/globals.asm', {
    flags: append ? 'a' : 'w'
  });
  ...
  const code = fs.createWriteStream('build/8080/' + filename + '_code.asm');
  ...
  lineReader.on('line', line => {
    ...
    if (/SECTION/.test(line)) {
      ...
      if (hasProp(ignoreFunctions, name)) {
        ...
        functionOutputStreams.push(fs.createWriteStream('build/8080/s/' + name + '.asm'));
      ...
      if (activeStream) {
        ...
        switch (name2) {
          ...
          case '_petsciiBytes':
            ...
            if (!hasProp(byteMaps, name2)) {
              rodataOutputStreams.push(fs.createWriteStream('build/8080/ro/' + name2 + '.asm'));
        ...
        if (rodataType === 1 || rodataType === 3) {
          ...
          rodataOutputStreams.push(fs.createWriteStream('build/8080/ro/' + name + (name === 'i_1' ? '' : '') + '.asm'));
...
function updateName (elem) {
  ...
  if (hasMatches) {
    execSync(
      'echo "\tSECTION rodata_compiler" >> build/8080/s/' + elem[0] + ' && ' +
        'cat ' +
          shellEscape(rodataLabels.filter(label => label[1]).map(label => 'build/8080/ro/' + label[0] + '.asm')) +
          ' >> build/8080/s/' + elem[0] + ' && ' +
        'echo "\tSECTION code_compiler" >> build/8080/s/' + elem[0]
    );
  }
...
function addDefines (filename, filenames, folderName, pageMode) {
  ...
  execSync(
    ...
    {
      cwd: path.join(__dirname, 'build/8080', 's')
    }
  );
