...
CFStringRef getSystemEncodedString(char* inputStr) {
  CFStringRef text;
  CFStringEncoding enc;
  CFIndex neededLen;
  CFIndex usedLen;
  ...
  if(converted = CFStringGetCStringPtr(text, enc)) {
    operation(converted, strlen(cstr));
  }
  else {
    neededLen = 0;
    usedLen = 0;
    range = CFRangeMake(0, CFStringGetLength(text));
    ...
    reallocMsg(&converted, neededLen + 1);
    ...
    operation(converted, (size_t)usedLen);
