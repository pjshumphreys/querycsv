...
%token CURSOR DECIMAL DECLARE DEFAULT DELETE DESC DISTINCT DOUBLE ELSE END
%token ERROR ESCAPE EXISTS FETCH FLOAT FOR FOREIGN FOUND FROM GOTO
...
%token COBOL FORTRAN PASCAL PLI C ADA ALLOCFAIL
...
scalar_exp:
  ...
  | CONCAT '(' scalar_exp ',' scalar_exp ')' { $$ = parse_scalar_exp(queryData, $3, EXP_CONCAT, $5); }
	|	'+' scalar_exp %prec UMINUS { $$ = parse_scalar_exp(queryData, $2, EXP_UPLUS, NULL); }
...
table_references:
    STRING AS NAME { parse_table_factor(queryData, FALSE, $1, $3); }
  | join_table
  ;
  
join_table:
    table_references JOIN STRING AS NAME { parse_table_factor(queryData, FALSE, $3, $5); } optional_join_condition
  | table_references LEFT JOIN STRING AS NAME { parse_table_factor(queryData, TRUE, $4, $6); } join_condition
  ;
...
join_condition:
   ON search_condition { parse_where_clause(queryData, $2); }
  ;
...
where_clause:
		WHERE search_condition { parse_where_clause(queryData, $2); }
	;

search_condition:
		search_condition AND search_condition { $$ = parse_scalar_exp(queryData, $1, EXP_AND, $3); }
	|	search_condition OR search_condition { $$ = parse_scalar_exp(queryData, $1, EXP_OR, $3); }
	|	NOT search_condition { $$ = parse_scalar_exp(queryData, $2, EXP_NOT, NULL); }
	|	'(' search_condition ')' { $$ = $2; }
	|	predicate { $$ = $1; }
	;
...
comparison_predicate:
		scalar_exp COMPARISON scalar_exp { $$ = parse_scalar_exp(queryData, $1, $2+EXP_EQ, $3); }
	;

in_predicate:
		scalar_exp IN '(' atom_commalist ')' { $$ = parse_in_predicate(queryData, $1, FALSE, $4); }
	|	scalar_exp NOT IN '(' atom_commalist ')' { $$ = parse_in_predicate(queryData, $1, TRUE, $5); }
	;

atom_commalist:
		literal { $$ = parse_atom_commalist(queryData, NULL, $1); }
	|	atom_commalist ',' literal { $$ = parse_atom_commalist(queryData, $1, $3); }
	;
...
ordering_spec:
		scalar_exp opt_asc_desc { parse_ordering_spec(queryData, $1, $2); }
	|	ordering_spec ',' scalar_exp opt_asc_desc { parse_ordering_spec(queryData, $3, $4); }
	;
