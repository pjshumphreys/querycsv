...
void getBytes(char** value, int* bytesRead) {
...
void screenio() {
  ...
  long* codePointBufferHead = NULL;
  int codePointBufferHeadLength = 0;

  long* codePointBufferReversedTail = NULL;
  int codePointBufferTailLength = 0;

  int state = 0;  //0 - not yet got start position, 
  ...
  for( ; ; ) {
    getBytes(&stdinBuffer, &stdinBufferLength);

    if(stdinBufferLength < 1) {
      if(esc) {
    ...
    for(i = 0; i < stdinBufferLength; i++) {
      switch(stdinBuffer[i]) { /* 1 byte inputed */
        ...
        default:
          ...
          write(STDOUT_FILENO, &(stdinBuffer[i]), 1);
        break;
      }
    }
  }
